...
if (!modelRoot || waves.length === 0) throw new Error('Usage: node scripts/evaluate-sensevoice.mjs <model-root> <wave...>')

const recognizer = sherpaOnnx.createOfflineRecognizer({
  modelConfig: {
    senseVoice: { model: join(modelRoot, 'model.int8.onnx'), language: 'auto', useInverseTextNormalization: 1 },
...
for (const filename of waves) {
  ...
  results.push({
    ...
    text: String(recognizer.getResult(stream)?.text ?? '').replace(/<\|[^|>]+\|>/gu, '').trim(),
  })
  stream.free()
}
recognizer.free()
